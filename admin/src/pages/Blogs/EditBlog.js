...
const EditBlog = () => {
   ...
   const handleDelete = () => {
      deleteBlog(data)
         .then((res) => {
            if (res) {
               toast.success(res?.message ?? "Ads deleted Successfully");
               navigate('/ads')
            }
         })
         .catch((err) => {
   ...
   const handleSubmit = () => {
      try {
         ...
         editBlogs(formData)
            .then((res) => {
               if (res) {
                  toast.success(res?.message ?? "Ads added Successfully");
                  navigate('/ads')
               }
            })
            .catch((err) => {
      ...
      >
         <Box sx={{ flexGrow: 1 }} display={'flex'} justifyContent={'center'}>
            <Grid container spacing={2} maxWidth={600} py={5}>
               ...
               <Grid item xs={12}>
                  <Button onClick={handleSubmit}>Update Ads</Button>
                  <Button color="secondary" onClick={handleDelete}>Delete Ads</Button>
               </Grid>
               <Grid item xs={12}>
